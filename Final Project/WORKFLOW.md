...
### 3.5 What counts as "functional data" — no precomputed tuning parameters
The brief constrains us to functional data only. We interpret this strictly: our inputs are the **raw functional traces and behavioral channels**, not precomputed properties derived from them by other models or by other people's analysis pipelines.
...
- **Disallowed as features.** Precomputed tuning parameters from `digital_twin_properties_bcm_coreg_v4.csv` — preferred orientation (`pref_ori`), preferred direction (`pref_dir`), `gOSI`, `gDSI`, the published oracle correlation, and any related index. These are tuning quantities the BCM digital twin model already computed for each neuron. Using them as features would (i) outsource the feature engineering to someone else's model, (ii) carry that model's specific assumptions and biases into our results, and (iii) defeat the scientific purpose of the project, which is *our* attempt to recover identity from raw functional signal.
- **Disallowed as labels.** Predicted cell-type tables — `baylor_log_reg_cell_type_coarse_v1.csv`, `baylor_gnn_cell_type_fine_model_v2.csv`, `cg_cell_type_calls.csv`, `cell_type_multifeature_combo.csv` — are not used as targets. Our ground truth comes from the AIBS metamodel labels (`aibs_metamodel_celltypes_v661.csv` with corrections), which are the consensus structural labels.
- **Allowed only as confound covariates, never as features.** `cc_abs` and other quality metadata derived from the digital twin appear only in the confound baselines (Section 6.5) and as quantities our features must beat — never as model inputs.

**Practical corollary.** Every tuning-like quantity that appears in the project (preferred orientation, OSI/DSI, response selectivity, sparseness, RF kernel) is computed by us from the raw traces and the stimulus video. If we want a "preferred orientation" feature for a neuron, we estimate it ourselves on the trial-averaged responses to Monet2 stimuli — we do not read it off a CSV.
